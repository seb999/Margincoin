    ...
    public class OrderController : ControllerBase
    {
        ...
        public bool OpenOrder([FromBody] Order order)
        {
            try
            {
                ...
                order.Fee = Math.Round((order.OpenPrice * order.QuantityBuy) / 100) * 0.1;
        ...
        public bool CloseOrder(int id, double closePrice)
        {
             try
            {
                ...
                myOrder.Fee = myOrder.Fee + Math.Round((closePrice * myOrder.QuantitySell) /100) * 0.1;
                myOrder.IsClosed = 1;
                myOrder.Profit = Math.Round((closePrice - myOrder.OpenPrice) * myOrder.QuantitySell);
